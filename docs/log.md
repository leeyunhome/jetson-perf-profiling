...
- `fib`의 어셈블리를 명령어 단위로 확인: 전체 함수 본문이 하나의 주소(`0x758`)를 공유하므로, `fib(n-1)` 호출부(`bl` at 784, 1.41%)와 `fib(n-2)` 호출부(`bl` at 794, 1.47%)의 샘플 비율이 거의 동일하게 나옴.
  - **한계**: annotate는 "어느 재귀 가지가 더 뜨거운가"를 답하지 못한다. 두 분기 모두 같은 함수 본문을 실행하므로 명령어 단위로는 구분이 안 되고, Flame Graph에서 본 비대칭(왼쪽 가지가 더 깊이 파고드는 뾰족탑)은 **호출 트리의 깊이/횟수** 문제이지 명령어 코스트 문제가 아님 — Flame Graph(호출 구조)와 annotate(명령어 핫맵)는 서로 답할 수 있는 질문이 다르다.
- `cmp w0, #0x1`이 40.11%로 비정상적으로 높게 나옴. `cmp`는 원래 1사이클짜리 단순 연산이라 그대로 믿기 어려움.
  - **해석**: ARM PMU의 샘플 스큐(skid) — 인터럽트 기반 샘플링이 실제 정지 지점보다 몇 명령어 뒤에 귀속되는 현상 — 로 추정. **annotate의 명령어별 퍼센트는 "정확한 그 명령어의 비용"이 아니라 "그 근처가 핫스팟"으로 읽어야 한다**는 실전 캐비어트.
